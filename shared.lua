FX = FX or {}
FX.Shared = FX.Shared or {}

FX.Shared.SpawnLocation = { x = -1037.79, y = -2737.78, z = 20.17, h = 100 }
...
FX.Shared.IsJobVaild = function(job)
    return FX.Shared.Jobs[job] ~= nil
end

FX.Shared.IsJobGradeVaild = function(job, grade)

    if not job or not FX.Shared.Jobs(job) then
        return false
    end

    for k,v in pairs(FX.Shared.Jobs[job][grades]) do
        if v == grade then
            return true
        end
    end
...
FX.Shared.GetModule = function(module)
    if FX[module] then 
        return FX[module]
    else
...
FX.Shared.AddModule = function(module, tbl)
    FX[module] = tbl
end

FX.Shared.ConsoleLog = function(text, module)
    if module then
        print("^1[FX " .. module .. "]^7 " .. text)
    else
        print("^1[FX]^7 " .. text)
    end
end

FX.Shared.GetIdentifier = function(source, type)
...
FX.Shared.HasValue = function(table, value)
...
FX.Shared.GetIndex = function(table, value)
	for k, v in pairs(table) do
		if v == value then
			return k
		end
...
Citizen.CreateThread(function()
    while true do
        ...
        if exports and exports[GetCurrentResourceName()] then
            TriggerEvent("fxbase:exportsReady", FX)
            print("^2[fxbase] ^7Ready!")
            return
        end
    end
end)

exports('AddModule', FX.Shared.AddModule)
exports('GetModule', FX.Shared.GetModule)
